...
def chunk_by_pages(
    ...
    """
    Page-aware chunking: respect page boundaries first, then split large pages
    into sub-chunks of `words_per_chunk` words with overlap.
    This prevents large PDFs from collapsing into very few chunks.
    """
    all_chunks = []
...
def process_document(
    ...
    For PDFs with 'words' strategy, page-aware chunking is used so that
    large PDFs (e.g. 300 pages) produce a proportionate number of chunks
    instead of collapsing into 10-15.
    """
    ...
    if ext == ".pdf" and chunk_strategy in ("words", "pages"):
        ...
        # Original approach for PPTX or non-words strategies
