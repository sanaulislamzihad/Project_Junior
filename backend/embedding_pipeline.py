...
_MODEL_REPO_CACHE_DIR = os.path.join(
    MODEL_CACHE_DIR,
...
def _configure_model_cache_env() -> None:
    # Keep all HF/transformers cache files inside the project so they survive restarts.
...
def _find_cached_model_snapshot() -> Optional[str]:
    # Return a fully cached snapshot path so we can force local-only loading.
    snapshots_dir = os.path.join(_MODEL_REPO_CACHE_DIR, "snapshots")
    ...
    for snapshot_name in sorted(os.listdir(snapshots_dir), reverse=True):
        ...
        if not os.path.isdir(snapshot_path):
            continue
        if all(os.path.isfile(os.path.join(snapshot_path, name)) for name in _MODEL_REQUIRED_FILES):
            return snapshot_path
    return None


def _get_model():
    # Lazy-load the sentence transformer model on first use; prefer local cached snapshot for offline runs.
    global _MODEL
    if _MODEL is None:
        from sentence_transformers import SentenceTransformer
        os.makedirs(MODEL_CACHE_DIR, exist_ok=True)
        _configure_model_cache_env()
        cached_snapshot = _find_cached_model_snapshot()
        try:
            ...
            raise RuntimeError(
                "Embedding model cache not found. Connect to the internet once so the backend can download the model, then future runs will use the local cache."
            ) from exc
    return _MODEL
