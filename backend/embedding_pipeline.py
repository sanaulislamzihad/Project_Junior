...
def _select_document_matches(candidate_records: List[dict], limit: int = DEFAULT_MAX_DOC_MATCHES_PER_CHUNK) -> List[dict]:
    """Keep the best explainable match per document, then return the top documents."""
    best_by_document = {}
    ...
    if use_faiss:
        ...
        if index is not None and chunk_infos:
            ...
            for qi, row in enumerate(faiss_results):
                ...
                total_sem += primary_record["semantic_similarity"]
                total_lex += primary_record["lexical_similarity"]
                total_fp += primary_record["fingerprint_similarity"]
                total_overall += primary_record["combined_similarity"]
                matched_count += 1
