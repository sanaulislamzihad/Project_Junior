...
def locate_matched_sentences(pdf_sentences: List[Dict], matches: List[Dict]) -> List[Dict]:
    ...
    When the same sentence matches multiple sources, all source indices are tracked."""
    located: List[Dict] = []
        ...
        for sentence_index, sentence_match in enumerate(match.get("similar_sentences") or []):
            ...
            for target in _split_highlight_targets(query_sentence):
                ...
                if key in key_to_entry:
                    existing = key_to_entry[key]
                    if match_index not in existing["match_indices"]:
                        existing["match_indices"].append(match_index)
                    file_name = match.get("file_name")
                    if file_name and file_name not in existing["matched_file_names"]:
                        existing["matched_file_names"].append(file_name)
                    continue
                ...
                entry = {
                    ...
                    "matched_file_names": [match.get("file_name")] if match.get("file_name") else [],
...
def highlight_pdf_matches(input_pdf_path: str, matches: List[Dict], output_pdf_path: str) -> Dict:
    """
    Annotate the uploaded PDF with per-source colored highlights and numbered badges.
    Each source document gets a unique color; sentences matching multiple sources show
    all source numbers in their badge row (Turnitin-style).
    """
    ...
    with pymupdf.open(input_pdf_path) as doc:
        for row in located_sentences:
            ...
            match_indices = row.get("match_indices", [row.get("match_index", 0)])
            primary_idx = match_indices[0] if match_indices else 0
            hl_color = HIGHLIGHT_COLORS[primary_idx % len(HIGHLIGHT_COLORS)]
            ...
            if regions and match_indices:
                first_region = regions[0]
                first_rect = pymupdf.Rect(
                    first_region.rect if hasattr(first_region, "rect") else first_region
                )
                badge_x = first_rect.x0
                badge_h = 12
                font_sz = 7.5
                badge_y0 = max(page.rect.y0, first_rect.y0 - badge_h - 1)
                badge_y1 = badge_y0 + badge_h

                max_visible = 3
                visible = match_indices[:max_visible]
                overflow = len(match_indices) - max_visible

                for mi in visible:
                    b_color = BADGE_COLORS[mi % len(BADGE_COLORS)]
                    b_text = str(mi + 1)
                    b_w = max(14, 4 + len(b_text) * 5)
                    b_rect = pymupdf.Rect(badge_x, badge_y0, badge_x + b_w, badge_y1)
                    page.draw_rect(b_rect, color=b_color, fill=b_color)
                    text_x = badge_x + (b_w - len(b_text) * 3.8) / 2
                    text_y = badge_y0 + font_sz + 1.5
                    page.insert_text(
                        pymupdf.Point(text_x, text_y),
                        b_text, fontsize=font_sz, color=(1, 1, 1),
                    )
                    badge_x += b_w + 2
                if overflow > 0:
                    ov_text = f"+{overflow}"
                    ov_w = max(18, 4 + len(ov_text) * 5)
                    ov_rect = pymupdf.Rect(badge_x, badge_y0, badge_x + ov_w, badge_y1)
                    page.draw_rect(ov_rect, color=(0.4, 0.4, 0.4), fill=(0.4, 0.4, 0.4))
                    text_x = badge_x + (ov_w - len(ov_text) * 3.8) / 2
                    text_y = badge_y0 + font_sz + 1.5
                    page.insert_text(
                        pymupdf.Point(text_x, text_y),
                        ov_text, fontsize=font_sz, color=(1, 1, 1),
                    )
