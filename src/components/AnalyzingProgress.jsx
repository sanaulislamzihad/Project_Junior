...
const AnalyzingProgress = ({ jobId, onComplete, onError, title = "Analyzing Document...", subtitle = "Checking for AI & Plagiarism matches", hideTitle = false, compact = false }) => {
        ...
        es.onerror = () => {
            ...
            fetchResultFallback().then((ok) => {
                if (!ok && !doneRef.current) {
                    // Job still running but SSE dropped (tab was backgrounded, network hiccup).
                    // Reconnect after a short delay instead of showing a permanent error.
                    setStage("Reconnecting\u2026");
                    reconnectTimerRef.current = setTimeout(() => {
                        if (!doneRef.current) openSSE();
                    }, 3000);
                }
            });
        };
    ...
    useEffect(() => {
        ...
        // Check if the job is already complete (e.g. component remounted after
        // page navigation or tab switch). If so, call onComplete immediately
        // without waiting for SSE events.
        fetchResultFallback().then((done) => {
            if (!done) openSSE();
        });
