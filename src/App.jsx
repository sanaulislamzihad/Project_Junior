...
function MainApp() {
  ...
  const handleCheckDocument = async (file) => {
    ...
    // Always search 'both' repos when user has personal docs, else fallback to selection
    ...
    // If only university selected but user is teacher, still send 'both' so their
    // personal repo is also searched (Turnitin always checks all available sources)
    ...
    if (user?.id) formData.append('user_id', String(user.id)); // always send so 'both' queries work
    try {
      const response = await axios.post('http://localhost:8000/analyze', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
